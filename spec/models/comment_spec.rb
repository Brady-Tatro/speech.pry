...
describe Comment do
  ...
  it 'is valid if it recieves a time' do
    if user.save && speech.save
      expect(Comment.new(user_id: user.id, speech_id: speech.id, time: "20:00")).to be_valid
    end
  end
  it 'should fail if not passed anything' do
    if user.save && speech.save
      expect(Comment.new(user_id: user.id, speech_id: speech.id, time: "")).to_not be_valid
    end
  end
end
